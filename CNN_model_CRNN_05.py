...
def test_train_split(X,Y,proportion=0.8):
    ...
    X_train = shuffle(X_train,random_state=11)
    X_test = shuffle(X_test,random_state=11)

    y_train = Y[:split]
    y_test = Y[split:]

    y_train = shuffle(y_train,random_state=11)
    y_test = shuffle(y_test,random_state=11)
...
def Model(num_classes, input_shape):
    ...
    ''' Simple Model Low number of Filters'''
    ...
    #Hidden Layer 2
    model.add(Conv2D(128, (3, 3), border_mode='same',kernel_initializer='he_normal', bias_initializer=Constant(0.01)))
    model.add(BatchNormalization(axis=sample_axis))
    model.add(ELU())
    model.add(MaxPooling2D(pool_size=(3, 3), strides=(3, 3)))
    model.add(Dropout(0.3))
